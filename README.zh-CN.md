...
让任意 CLI agent（Claude Code / Codex / Gemini / Qwen / Kimi / OpenCode...）互相评审你的技术方案与代码 diff，直到全票通过。

单个模型自评是循环论证；**多家不同厂商的模型交叉质询你的方案，才叫评审团。**（一家评审员即可与 host 形成交叉；2-3 家不同厂商时交叉验证最强）
...
MIT License。评审员 = agent（不选模型），实验性适配器（gemini/qwen/aider/qoder）欢迎社区验证提 PR。
